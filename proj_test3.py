...
class Gui(TK.Frame):    
	def __init__(self,master):
	        ...
	        self.cameraPosition = np.array([325, 250, -5])
		self.rotationAxis = np.array([0, 1, 0])
		self.rotationDegree = -30
	...
	def movingPerspectiveProject(self):
		qp = quatmult(getQuaternionQ(self.rotationDegree, self.rotationAxis), [0, self.cameraPosition[0], self.cameraPosition[1], self.cameraPosition[2]])

		qpqPrime = quatmult(qp, getQuaternionQPrime(getQuaternionQ(self.rotationDegree, self.rotationAxis)))
		cameraFrame2_position = [qpqPrime[1], qpqPrime[2], qpqPrime[3]]

		quatmat_1 = quat2rot(getQuaternion(self.rotationDegree, self.rotationAxis))

		newPoints = np.transpose(np.matrix(self.projectingXYZ))
		currentPointsColor = np.transpose(np.matrix(self.projectingRGB))
		...
		for j in xrange(len(newPoints)):
			...
			if (325 + image1_u >= 0 and 325 <= 650) and (250 + image1_v >= 0 and 250 + image1_v <= 500):
				pixels[325 + image1_u, 250 + image1_v] = (currentPointsColor[0, j], currentPointsColor[1, j], currentPointsColor[2, j])
	...
	def addPolygon(self):
		...
		newPoints = np.transpose(np.matrix(np.array(newPoints)))
		
		self.projectingXYZ = np.concatenate((self.projectingXYZ, np.array(newPoints)))
